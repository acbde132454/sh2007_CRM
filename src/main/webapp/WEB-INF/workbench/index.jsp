...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 70%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" data-dismiss="modal" onclick="uploadPwd($('#newPwd').val())">更新</button>
				</div>
			</div>
		</div>
	</div>
			...
			<ul>
				<li class="dropdown user-dropdown">
					<a href="javascript:void(0)" style="text-decoration: none; color: white;" class="dropdown-toggle" data-toggle="dropdown">
						<img src="${sessionScope.user.img}" id="photo" style="width: 30px;height: 30px;border-radius: 100%" /> </span> ${sessionScope.user.name} <span class="caret"></span>
					</a>
	...
	<script>
		...
		//异步更新密码
		function uploadPwd(newPwd) {
			$.post("/crm/settings/user/updatePwd",{
				'loginPwd' : newPwd
			},function(data){
				if(data.ok){
					layer.alert(data.mess, {icon: 1});
					//attr:修改自定义属性 prop:修改固有属性
				}
			},'json');
		}
	</script>
